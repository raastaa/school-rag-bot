...
def upsert_chunks(vectors: List[List[float]], payloads: List[Dict[str, Any]]):
    cli = get_client()
    points = [PointStruct(id=payloads[i].get("id"), vector=vectors[i], payload=payloads[i]) for i in range(len(vectors))]
    cli.upsert(collection_name=QDRANT_COLLECTION, points=points)

def search(query_vector: List[float], top_k: int = 5, source_filter: Optional[str] = None):
    cli = get_client()
    flt = None
    if source_filter:
        flt = Filter(must=[FieldCondition(key="source_group", match=MatchValue(value=source_filter))])
    res = cli.search(collection_name=QDRANT_COLLECTION, query_vector=query_vector, limit=top_k, query_filter=flt)
